...
>(({ className, ...props }, ref) => (
  <SwitchPrimitives.Root
    className={cn(
      'peer inline-flex h-7 w-12 shrink-0 cursor-pointer items-center rounded-full border-4 border-ink/15 bg-ink/15 shadow-pop-sm transition-colors focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-sun/70 disabled:cursor-not-allowed disabled:opacity-50 data-[state=checked]:border-grape/40 data-[state=checked]:bg-grape',
      className
    )}
    {...props}
    ref={ref}
